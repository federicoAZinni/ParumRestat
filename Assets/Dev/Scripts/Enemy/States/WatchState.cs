...
public class WatchState : MonoBehaviour, IState
{
    ...
    private IEnumerator UpDetection()
    {
        ...
        {
            _currentDetectionValue += (Time.deltaTime / _maxDetectionValue) * (enemyAI.distanceVision - Vector3.Distance(transform.position, enemyAI.target.transform.position));
    ...
    private void Catched()
    {
        ...
        enemyAI.enemiesManager.OnEnemyCatchPlayer(enemyAI, 3);
