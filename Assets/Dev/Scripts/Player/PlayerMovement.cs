...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField, Tooltip("M s es m s r pido"), Range(0.1f, 10f)] float rotSpeed;
    ...
    public AudioSource WalkinCouch;
    ...
    private void Move()
    {
        ...
        if (Input.GetButtonDown("Horizontal"))
        {
            ...
            if (!onCrouch)
            {
                WalkinCouch.Stop(); // 🔸 detiene el otro sonido
                Walkin.Play();
            }
            else
            {
                Walkin.Stop(); // 🔸 detiene el otro sonido
                WalkinCouch.Play();
            }
        }
        if (Input.GetButtonDown("Vertical"))
